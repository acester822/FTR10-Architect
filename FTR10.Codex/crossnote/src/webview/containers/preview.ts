...
const PreviewContainer = createContainer(() => {
  ...
  const [kaleidoscope, setKaleidoscope] = useState<boolean>(
    () => localStorage.getItem('mpae-kaleidoscope') === 'on',
  );
  ...
  useEffect(() => {
    localStorage.setItem('mpae-kaleidoscope', kaleidoscope ? 'on' : 'none');
    document.documentElement.setAttribute(
      'data-mpae-kaleidoscope',
      kaleidoscope ? 'on' : 'none',
    );
  }, [kaleidoscope]);

  const toggleKaleidoscope = useCallback(() => {
    setKaleidoscope((prev) => !prev);
  }, []);
