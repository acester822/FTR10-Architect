...
class NotebooksManager {
  ...
  private async loadNotebookConfig(
    ...
    // ThemeCustomizer overrides — stored in extension global storage, never in ~/.ftr10
    let userStyleCss = '';
    try {
      const userStyleUri = vscode.Uri.joinPath(this.context.globalStorageUri, 'user-style.css');
      userStyleCss = Buffer.from(await vscode.workspace.fs.readFile(userStyleUri)).toString('utf8');
    } catch {
    ...
    return {
      ...
      ...workspaceConfig,
      globalCss:
        (globalConfig.globalCss ?? '') + (workspaceConfig.globalCss ?? '') + userStyleCss,
      previewTheme,
    };
  }
