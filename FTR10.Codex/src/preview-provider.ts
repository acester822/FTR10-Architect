...
export class PreviewProvider {
  ...
  private static publishServers: Map<
    string,
    { server: http.Server; rootDir: string; url: string }
  > = new Map();
  ...
  private async servePublishedFile(dest: string): Promise<string> {
    const rootDir = path.dirname(dest);
    const existing = PreviewProvider.publishServers.get(rootDir);
    if (existing) {
      return existing.url;
    }
    ...
    const server = http.createServer(async (request, response) => {
      try {
        const requestPath = request.url
          ...
          : '/';
        let filePath = requestPath === '/' ? dest : path.join(rootDir, requestPath);
        ...
        fileStream.on('error', (error) => {
          ...
          response.end(`I/O error: ${error.message}`);
        });
      } catch (error) {
        response.writeHead(500, { 'Content-Type': 'text/plain' });
    ...
    const url = await new Promise<string>((resolve, reject) => {
      ...
      server.listen(0, '0.0.0.0', () => {
        ...
        if (address && typeof address === 'object') {
          ...
          resolve(`http://127.0.0.1:${port}/${encodeURIComponent(path.basename(dest))}`);
        } else {
          reject(new Error('Failed to start publish server'));
        }
      });
    });

    PreviewProvider.publishServers.set(rootDir, { server, rootDir, url });
    return url;
  }
  ...
  public markdownExport(sourceUri: Uri) {
    ...
    if (engine) {
      engine
        ...
        .then((dest) => {
          vscode.window.showInformationMessage(
            `Document ${path.basename(dest)} was created at path: ${dest}`,
          );
        })
        .catch((error) => {
          vscode.window.showErrorMessage(error.toString());
  ...
  public async publish(sourceUri: Uri, publishUrl?: string) {
    const engine = this.getEngine(sourceUri);
    if (engine) {
      if (isVSCodeWebExtension()) {
        vscode.window.showErrorMessage(
          `Publish is not supported in MPE web extension.`,
        );
        return;
      }
      ...
      try {
        ...
        if (publishUrl && /^https?:\/\//i.test(publishUrl)) {
          const html = await fs.promises.readFile(dest, 'utf-8');
          await this.uploadToPublishUrl(publishUrl, html);
          vscode.window.showInformationMessage(
            `Published ${path.basename(dest)} to ${publishUrl}`,
          );
          return;
        }

        const url = await this.servePublishedFile(dest);
        vscode.env.openExternal(vscode.Uri.parse(url));
        vscode.window.showInformationMessage(
          `Published ${path.basename(dest)} and serving it at ${url}`,
        );
      } catch (error) {
        vscode.window.showErrorMessage(
  ...
  private async uploadToPublishUrl(url: string, content: string) {
      ...
      const req = httpModule.request(
        ...
        (res: any) => {
          ...
          res.on('end', () => {
            reject(
              new Error(
                `Publish failed: ${res.statusCode} ${res.statusMessage} ${data}`,
              ),
            );
          });
        },
